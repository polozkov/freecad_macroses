...
CONST_FILE_NAME_OUTPUT = "CANAPE_MULTI_COLOR_V1.step"
# путь к шрифту
...
def f_final_combine():
    ...
    for i in range(LEN):
        i_shape_in_bb = m1_card_start.f_fit_shape_to_bbox_3d(my_info["arr_fused_shapes"][i], *f_calculate_bb(0), union_svg_shape)
        m1_card_start.f_add_obj(f_i_name(i), i_shape_in_bb, my_info["arr_colors"][i], doc, M44)
    ...
    shape_total_svg = m1_card_start.f_shape_multi_fuse([doc.getObject(f_i_name(i)).Shape for i in list(range(LEN))])
    shape_array_for_cut = [shape_qr_code_in_bb, shape_text_in_bb, shape_total_svg]
